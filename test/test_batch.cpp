    ...
    void test_mulhilo_impl(std::true_type /*integral*/) const
    {
        ...
        // vmin*vmax, vmin*-1 — the classic signed-overflow corners).
        {
            ...
            for (std::size_t i = 0; i < size; ++i)
            {
                switch (i % 8)
                {
                case 0: a[i] = vmax; b[i] = vmax; break;
                case 1: a[i] = vmin; b[i] = vmin; break;
                case 2: a[i] = vmin; b[i] = vmax; break;
                case 3: a[i] = vmax; b[i] = static_cast<value_type>(is_signed ? -1 : vmax); break;
                case 4: a[i] = static_cast<value_type>(is_signed ? -1 : vmax);
                        b[i] = static_cast<value_type>(is_signed ? -1 : vmax); break;
                case 5: a[i] = vmin; b[i] = static_cast<value_type>(is_signed ? -1 : 1); break;
                case 6: a[i] = static_cast<value_type>(vmax / 2 + 1);
                        b[i] = static_cast<value_type>(vmax / 2 + 1); break;
                case 7: a[i] = static_cast<value_type>(1); b[i] = vmax; break;
                }
            }
        ...
        // catch lane-wise bugs in 32/64-bit emulated mulhi paths).
        {
            ...
            for (std::size_t i = 0; i < size; ++i)
            {
                ...
                if ((ua & half_mask) == 0) ua |= static_cast<UT>(1);
                if ((ub & half_mask) == 0) ub |= static_cast<UT>(1);
